...
9. Start a long run, **close the side panel**, wait ten seconds, reopen it while the run is still
   ...
   **Expected:** the activity line, **Stop** and **Queue** are showing; the rows produced while the
   ...
   bubble still marked queued, in a chat that is neither running nor interrupted — which takes a
   panel page dying in the instant before the background notices its port closed. When it does
   appear it closes those rows ("result not shown here · the model received it") and says what is
   intact: the conversation the model sees, the draft and saved mods. It is never shown for a run
   that is still going, and an interrupted run says "This run was interrupted." instead.
